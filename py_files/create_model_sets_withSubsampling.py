...
def main(feats_data, manual_grades, role_name, p = True, N=100):
    ...
    # ----------------------------------- subsample to create a training set with N of each grade
    # N is number of crew to take from each grade bucket during training. Subsampling to help even out grade distribution in training set
    sampled_df = pd.DataFrame(columns = split_df.columns)
    for grade_val in range(5):
        g_df = split_df[split_df['grade'] == grade_val]
        if len(g_df)>0:
            if grade_val>2:
                for draw in range(1,N+51):  #take extra samples from grades 3, 4
                    d = np.random.randint(low=0,high=len(g_df))
                    sampled_df = sampled_df.append(g_df.iloc[d, :])
            else:
    ...
    np.random.seed(0)
    shuffle = np.random.permutation(np.arange(len(sampled_df['grade'].values)))

    features = sampled_df.drop(['CharacterDesignId', 'grade'], axis = 1).values[shuffle]
    labels = sampled_df['grade'].values[shuffle]
